...
export function CashCounter() {
  ...
  const RenderSection = ({ title, icon: Icon, data }: { title: string, icon: any, data: number[] }) => (
    <div className="space-y-3">
      <div className="flex items-center gap-2 px-1">
        ...
        <h3 className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest">{title}</h3>
      </div>
      {data.map((val) => (
        <div key={`${title}-${val}`} className="bg-bg-card p-3 sm:p-4 rounded-[20px] sm:rounded-2xl flex items-center justify-between gap-2 sm:gap-4 border border-white/5 hover:border-primary/20 transition-all group overflow-hidden">
          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            <div className="w-10 sm:w-12 h-7 sm:h-8 rounded-lg bg-zinc-900 border border-white/5 flex items-center justify-center font-bold text-[10px] sm:text-xs group-hover:text-primary transition-colors">
              ₹{val}
            </div>
          ...
          <input
            ...
            className="bg-zinc-950/50 border-b border-white/5 w-16 sm:w-24 text-right focus:border-primary outline-none font-mono text-base sm:text-lg py-1 px-1 sm:px-2 text-white"
            placeholder="0"
          />
          
          <div className="w-24 sm:w-32 text-right">
            <span className="text-primary font-mono font-bold text-xs sm:text-sm">
  ...
  return (
    <div className="space-y-6 sm:space-y-8 pb-10">
      <div className="bg-gradient-to-br from-primary/20 to-bg-card rounded-[28px] sm:rounded-[32px] p-6 sm:p-8 border border-primary/20 relative overflow-hidden shadow-2xl">
        <div className="relative z-10">
          <p className="text-slate-400 text-[10px] sm:text-xs uppercase tracking-[0.2em] font-bold mb-1">Grand total cash</p>
          <p className="text-3xl sm:text-5xl font-mono font-bold text-primary drop-shadow-[0_0_15px_rgba(37,99,235,0.3)]">{formatCurrency(total)}</p>
          <div className="mt-4 flex gap-4">
            <button onClick={handleCopy} className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-500 flex items-center gap-1 hover:text-white transition-colors">
              <Copy className="w-3 h-3" /> {T('copy')}
            </button>
            <button onClick={handleShare} className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-500 flex items-center gap-1 hover:text-white transition-colors">
              <Share2 className="w-3 h-3" /> {T('share')}
            </button>
          </div>
        </div>
        <IndianRupee className="absolute top-1/2 right-2 w-24 h-24 sm:right-4 sm:w-32 sm:h-32 text-primary/5 -rotate-12" />
      </div>
